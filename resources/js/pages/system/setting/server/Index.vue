<template>
    <vue-simplebar>
        <div class="page-container" v-loading="loading">
            <div class="page-filter-option">
                <el-form :inline="true" :model="filterOption" class="d-flex flex-wrap">
                    <el-form-item>
                        <el-input :placeholder="$t('system.setting.server.name')" prefix-icon="el-icon-search" clearable
                                  v-model="filterOption.keyword" @change="filterChange"></el-input>
                    </el-form-item>
                    <el-form-item class="el-form-item-small">
                        <el-select :placeholder="$t('system.setting.server.deleted')" v-model="filterOption.deleted" clearable @change="filterChange">
            ...
            <div class="page-container-inner-table flex-auto">
                <el-table style="width: 100%"
                    ...
                    <el-table-column min-width="150" :label="$t('system.setting.server.name')" prop="name">
                        <template slot-scope="scope">
                            <span v-html="$options.filters.hsFilterKeyword(scope.row.name, filterOption.keyword)"></span>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="320" :label="$t('system.setting.server.mysql_agency.title')" prop="mysql_agency" show-overflow-tooltip>
                        <template slot-scope="scope">{{ scope.row.mysql_agency }}</template>
                    </el-table-column>
                    <el-table-column min-width="220" :label="$t('system.setting.server.updated')" prop="updated" sortable="custom" :sort-orders="['descending', 'ascending']">
                        <template slot-scope="scope">
                            <component-page-timestamp :timestamp="scope.row.updated"></component-page-timestamp>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="100" :label="$t('system.setting.server.deleted')" prop="deleted">
                        <template slot-scope="scope">
                    ...
                    <el-table-column min-width="100" :label="$t('system.setting.server.operation')" align="right" fixed="right">
                        <template slot-scope="scope">
                            <span class="operation-options-icon">
        ...
        <el-dialog :title="$t('system.setting.server.dialog.deleted')" center width="320px" :visible.sync="dialog.visible.deleted">
            <div class="text-center" v-html="$t('confirm.server', { status: !itemCache.deleted ? $t('action.off') : $t('action.on') })"></div>
            <div slot="footer">
        ...
        <el-dialog :title="$t('system.setting.server.dialog.destroy')" center width="340px" :visible.sync="dialog.visible.destroy">
            <div class="text-center" v-html="$t('confirm.server', { status: $t('action.delete') })"></div>
