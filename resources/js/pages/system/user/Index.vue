<template>
    <vue-simplebar>
        <div class="page-container" v-loading="loading">
            <div class="page-filter-option">
                <el-form :inline="true" :model="filterOption" class="d-flex flex-wrap">
                    <el-form-item>
                        <el-input :placeholder="$t('system.user.email')" prefix-icon="el-icon-search" clearable
                                  v-model="filterOption.keyword" @change="filterChange"></el-input>
                    </el-form-item>
                    <el-form-item class="el-form-item-small">
                        <el-select :placeholder="$t('system.user.deleted')" v-model="filterOption.deleted" clearable @change="filterChange">
            ...
            <div class="page-container-inner-table flex-auto">
                <el-table style="width: 100%"
                    ...
                    <el-table-column min-width="150" :label="$t('system.user.name')" prop="name"></el-table-column>
                    <el-table-column min-width="200" :label="$t('system.user.email')">
                        <template slot-scope="scope">
                            <span v-html="$options.filters.hsFilterKeyword(scope.row.email, filterOption.keyword)"></span>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="120" :label="$t('system.user.roles')">
                        <template slot-scope="scope">
                    ...
                    <el-table-column min-width="220" :label="$t('system.user.updated')" prop="updated" sortable="custom" :sort-orders="['descending', 'ascending']">
                        <template slot-scope="scope">
                            <component-page-timestamp :timestamp="scope.row.updated"></component-page-timestamp>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="100" :label="$t('system.user.deleted')" prop="deleted">
                        <template slot-scope="scope">
                            <el-tag effect="plain" size="mini" :type="!scope.row.deleted ? 'info' : 'danger'">
                                {{ $t('form.deletedList')[Number(!!scope.row.deleted)].label }}
                            </el-tag>
                            <span class="operation-options-icon" v-if="isSuperAdmin || !scope.row.isSuperAdmin">
                               <i class="el-icon-edit" @click="dialogItemDeleted(scope)"></i>
                            </span>
                        </template>
                    </el-table-column>
                    <el-table-column min-width="100" :label="$t('system.user.operation')" align="right" fixed="right">
                        <template slot-scope="scope">
        ...
        <el-dialog :title="$t('system.user.dialog.deleted')" center width="320px" :visible.sync="dialog.visible.deleted">
        ...
        <el-dialog :title="$t('system.user.dialog.destroy')" center width="340px" :visible.sync="dialog.visible.destroy">
    ...
    export default {
        ...
        methods: {
            ...
            destroy() {
                ...
                let loading = this.$loading();
                // 保存数据
                axios.post('/system/user/forceDestroy', {
                    id: this.itemCache.id
                }).then((response) => {
                    // loading 状态 close
                    loading.close();
