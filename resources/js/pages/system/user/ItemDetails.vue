<template>
    <el-drawer :title="$t('system.user.drawer.details')" size="400px" :visible.sync="drawerVisible" @close="$emit('update:visible', false)">
        <vue-simplebar>
            <div class="pt-5 pb-5" v-if="data.id">
                <el-form label-width="120px" class="el-form-detail">
                    <el-form-item label="ID">{{ data.id }}</el-form-item>
                    <el-form-item :label="$t('system.user.name')">{{ data.name }}</el-form-item>
                    <el-form-item :label="$t('system.user.email')">{{ data.email }}</el-form-item>
