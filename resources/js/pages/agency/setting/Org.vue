<template>
    <div class="h-100">
                    ...
                    <el-dropdown class="node-option"
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item command="create">
                                <i class="el-icon-folder-add"></i>{{ $t('agency.setting.tree.create') }}
                            </el-dropdown-item>
        ...
        <el-dialog :title="$t('agency.setting.tree.dialog.create')" width="400px" :visible.sync="dialog.visible.create" @submit.native.prevent>
            <div class="pr-5">
                <el-form :model="dataCreate" label-width="80px">
                    <el-form-item :label="$t('agency.setting.tree.dialog.parent')">
                        <el-tag type="info" size="mini">{{ dataCreate.parent_id }}</el-tag>
                        <span class="ml-1">{{ dataCreate.parent_name }}</span>
                    </el-form-item>
                    <el-form-item :label="$t('agency.setting.tree.dialog.name')" :error="Boolean(msg.errors['name']) ? msg.errors['name'][0] : ''" required>
        ...
        <el-dialog :title="$t('agency.setting.tree.dialog.update')" width="400px" :visible.sync="dialog.visible.update">
            <div class="pr-5">
                <el-form :model="dataUpdate" label-width="80px">
                    <el-form-item :label="$t('agency.setting.tree.dialog.sort')" :error="Boolean(msg.errors['sort']) ? msg.errors['sort'][0] : ''" required>
                        <el-input v-model="dataUpdate.sort" :placeholder="$t('agency.setting.tree.dialog.sortPlaceholder')"></el-input>
                    </el-form-item>
                    <el-form-item :label="$t('agency.setting.tree.dialog.name')" :error="Boolean(msg.errors['name']) ? msg.errors['name'][0] : ''" required>
        ...
        <el-dialog :title="$t('agency.setting.tree.dialog.destroy')" center width="320px" :visible.sync="dialog.visible.destroy">
    ...
    export default {
        ...
        methods: {
            getTreeList() {
                ...
                axios.get('/agency/setting/org/getList').then((response) => {
            ...
            treeNodeCreate() {
                ...
                // 保存数据
                axios.post('/agency/setting/org/store', this.dataCreate).then((response) => {
                    // loading 状态 close
                    loading.close();
                    // 逻辑处理
                    if (response.data.resp_msg.code == 200) {
                        ...
                        if (data.parent_id == 0) {
                            ...
                            this.$refs.treeOrg.append(data, this.nodeCache.node)
                        }
            ...
            treeNodeUpdate() {
                ...
                let loading = this.$loading();
                // 保存数据
                axios.post('/agency/setting/org/update', this.dataUpdate).then((response) => {
                    // loading 状态 close
                    loading.close();
                    // 逻辑处理
                    if (response.data.resp_msg.code == 200) {
            ...
            treeNodeDestroy() {
                ...
                axios.post('/agency/setting/org/destroy', {
                    ...
                    if (response.data.resp_msg.code == 200) {
                        ...
                        this.$message({
                            type: 'warning',
                            message: this.$t('messages.delete-tree-failed'),
                            showClose: true
                        });
                    } else {
                        this.$message({
